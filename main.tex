...
\begin{pf}{}
    ...
    \item[] It suffices to verify the $3$ subcategory conditions for \(\mathsf{D}\).  Indeed:
    \begin{enumerate}
...
\begin{pf}{(i)}
    ...
    \begin{enumerate}
        \item[$(\alpha)$] \(\mathrm{Obj}\left(c/\mathsf{C}\right):= \left\{f : c \to x \mid x \in \mathrm{Obj}(\mathsf{C})\right\}\) 
        \item[$(\beta)$] 
        \(\forall \substack{f:c\to x\\g:c\to y}\in\mathrm{Obj}\left(c/\mathsf{C}\right)\bm{\implies}\Hom_{c/\mathsf{C}}\left(f,g\right):=\{h : x \to y \in \Hom_{\mathsf{C}}\left(x,y\right)\mid g = h\circ f\}\)
        \item[$(\gamma)$] \(\forall\substack{\psi:f\to g\\\phi:g\to k} \bm{\implies} \phi\circ\psi \:\phi\circ_{\mathsf{C}}\psi\)
    \end{enumerate}
    It suffices to prove that the morphisms \underline{contain their identities}, are \underline{closed under composition}, and are \underline{associative}.  Indeed:
        \item (Identites) First notice that 
        
        From \((\beta)\), \(\substack{f:c\to x\\f:c\to x}\in\mathrm{Obj}\left(c/\mathsf{C}\right)\) gives\[\Hom_{c/\mathsf{C}}\left(f,f\right):=\{h : x \to x \in \Hom_{\mathsf{C}}\left(x,x\right)\mid f = h\circ f\}\]
        But here, \(h\) is precisely the identity \(1_f\:1_x \in \Hom_\mathsf{C}(x,x)\).
        \[\psi:f\to g \text{ and }\phi:g\to k \quad \text{for }f,g,h \in \mathrm{Obj}\left(c/\mathsf{C}\right)
        \text{with }
        \mathrm{cod}\left(f\right)=x\,,
        \mathrm{cod}\left(g\right)=y\,,
        \mathrm{cod}\left(k\right)=z\] 
        Then by \((\beta)\) we know \(\substack{\psi\:h_1\in \Hom_C(x,y)\\\phi\:h_2\in\Hom_C(y,z)}\) so since \(\mathsf{C}\) is a category, we have \(\phi\circ\psi\:h_2\circ h_1 \in \Hom_C(x,z)\).
        \item[] (Transitivity)
        A similar argument to the above interpreting the morphisms of \(c/\mathsf{C}\) as morphisms of \(\mathsf{C}\) and using the fact that \(\mathsf{C}\) is a category gives transitivity.
