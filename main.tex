...
\newtheoremstyle{spacious}% <name>
  {16pt}      % Space above
  {8pt}     % Space below  <-- increase this for more after-spacing
  {\color{solarized-base03}\normalfont} % Body font
  {}         % Indent amount
  {\color{solarized-base01}\bfseries}% Head font
  {\color{solarized-base01}.}        % Punctuation after head
...
\AtBeginEnvironment{dfn}{\color{solarized-blue}}

\newcommand{\m}[1]{\begingroup\color{solarized-magenta}#1\endgroup}
...
\renewcommand{\:}{{\color{solarized-magenta}\vcentcolon=}} % for pretty :=
...
\newcommand{\tbf}[1]{{\color{solarized-magenta}{\textbf{#1}}}}

% \DeclarePairedDelimiter{\ceil}{\lceil}{\rceil}
% \DeclarePairedDelimiter{\floor}{\lfloor}{\rfloor}

% Emphasis - pink
\renewcommand{\it}[1]{{\color{solarized-magenta}\textit{#1}}}
\renewcommand{\bf}[1]{{\color{solarized-magenta}\textbf{#1}}}

% Category - green
\renewcommand{\c}[1]{{\color{solarized-green}\mathsf{#1}}}

% Objects - blue
\newcommand{\A}{{\color{solarized-blue}A}}
\newcommand{\B}{{\color{solarized-blue}B}}
\newcommand{\C}{{\color{solarized-blue}C}}
\newcommand{\D}{{\color{solarized-blue}D}}
\newcommand{\E}{{\color{solarized-blue}E}}

\newcommand{\Ob}[1]{{\color{solarized-blue}\mathrm{Ob}\left(\c{#1}\right)}}
...
% Morphisms - yellow
\newcommand{\Hom}[3]{\color{solarized-yellow}{{\mathrm{Hom}}_{\color{solarized-green}#1}\left({\color{solarized-blue}#2},{\color{solarized-blue}#3}\right)}}
\newcommand{\1}[1]{{\color{solarized-yellow}{\mathrm{1}}_{\color{solarized-green}#1}}}
...
\begin{rmk}[Color Key]\phantom{a}
    \begin{itemize}
        \item \textcolor{solarized-magenta}{\textbf{Magenta}}: emphasis, definitions, and category names
        \item \textcolor{solarized-green}{\textbf{Green}}: categories
        \item \textcolor{solarized-blue}{\textbf{Blue}}: objects
        \item \textcolor{solarized-yellow}{\textbf{Yellow}}: morphisms
    \end{itemize}
\end{rmk}

\begin{dfn}
A \bf{category} $\c{C}$ consists of
\begin{enumerate}
\item a class of \color{solarized-blue}{\textbf{objects}}
\[
    \Ob{C}
\]
\item a function assigning to each ordered pair of objects a class of \color{solarized-yellow}{\textbf{morphisms}}:
\end{enumerate}


\[\begin{aligned}
    \forall \A,\B\in \Ob{C}\implies \exists\Hom{C}{A}{B}&\:\left\{\text{a class of morphisms from $A$ to $B$}\right\}\\
    \forall \A,\B,\C\in \Ob{C}\implies \exists\Hom{C}{A}{B}&\:\left\{\text{a class of morphisms from $A$ to $B$}\right\}\\ 
    \forall \A\in \Ob{C}\implies \exists \1{A} \in \Hom{C}{A}{A}&\:\left\{\text{a}\right\}\\
\end{aligned}\]
...
\[
\Hom{C}{B}{C}\times\Hom{C}{A}{B}
\to
\Hom{C}{A}{C}
\]
\end{enumerate}

satisfying associativity and identity laws.\labelenumi\theenumi
\end{dfn}

\begin{dfn} A \tbf{category} consists of 
    \begin{itemize}
        \item a collection of \textbf{objects} \m{X,Y,Z,\ldots}
...
%     \item[] Hence by the argument presented in \eqref{eq:star} we obtain \(g=h\).
% \end{pf}
...
%     \item[] \(\forall x,y\in\mathrm{Obj}\left(\mathsf{D}\right)\bm{\implies}\mathsf{D}\left(x,y\right):=\{f \in \mathsf{C}\left(x,y\right) \mid f \text{ is an isomorphism}\}\)
%     \end{enumerate}
    
%     \item[] It suffices to verify the $3$ subcategory conditions for \(\mathsf{D}\):
%     \begin{enumerate}
%     \item[(i)] No object has been removed, so domains and codomains of all \textit{iso}morphisms are still present
%     \item[(ii)] Identities are isomorphisms, so remain in our definition of \(\mathsf{D}\)
%     \item[(iii)] The composite of composible pairs of isomorphisms are isomorphisms that remain in \(\mathsf{D}\):\[(f^{-1}g^{-1})(gf)=1_X\]    
%     \end{enumerate}
% \end{pf}

% \begin{prm}[1.1.iii.(i)]
%     For any category \(\mathsf{C}\) and any object \(c\in \mathsf{C}\) show that there is a category \(c/ \mathsf{C}\) whose objects are morphisms \(f: c\to x\) with domain \(c\) and in which a morphism from \(f:c \to x\) to \(g:c \to y\) is a map \(h:x\to y\) between the codomains so that the triangle
%         \[
...
%         \item[] 
%         \(\forall \substack{f:c\to x\\g:c\to y}\in\mathrm{Obj}\left(c/\mathsf{C}\right)\bm{\implies}\Hom_{c/\mathsf{C}}\left(f,g\right):=\{h : x \to y \in \mathsf{C}\mid g = h\circ f\}\)
%         \item[] it suffices to prove that \(\mathrm{Mor}\left(c/\mathsf{C}\right)\) is closed under composition and contains identities, but this is true since:
...
%     For any category \(\mathsf{C}\) and any object \(c\in \mathsf{C}\) show that there is a category \(\mathsf{C}/c\) whose objects are morphisms \(f: x\to c\) with domain \(c\) and in which a morphism from \(f:x \to c\) to \(g:y \to c\) is a map \(h:x\to y\) between the domains so that \(f=gh\).
% \end{prm}
...
%     % \(\forall z \quad \forall g:z \to y \quad \exists h:z \to kx \st f\circ h =g \) \\
%     &&\\
% \hline
% \end{tabular}
